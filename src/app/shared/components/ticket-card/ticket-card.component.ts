...
export class TicketCardComponent {
  ...
  get managerMeta(): string {
    const m = this.ticket?.assigned_manager;
    if (!m) return '';
    const pos = m.position ? m.position.split(' ')[0] : '';
    const skills = m.skills ?? '';
    const load = m.current_workload ?? 0;
    return [pos, skills, `Load: ${load}`].filter(Boolean).join(' · ');
  }

  get hasAttachments(): boolean {
    const a = this.ticket?.attachments;
    return !!(a && a.trim() !== '' && a !== 'nan');
  }
